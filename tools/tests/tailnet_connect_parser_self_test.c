...
int main(void) {
    ...
    expect_absolute("GET http://nas.tailad3199.ts.net:4533/ HTTP/1.1\r\n"
    ...
    expect_result("GET /relative/only HTTP/1.1\r\nHost: host\r\n\r\n", 43,
            TAILNET_CONNECT_METHOD_NOT_ALLOWED);
    expect_result("GET ftp://host:443/ HTTP/1.1\r\n\r\n", 32,
            TAILNET_CONNECT_METHOD_NOT_ALLOWED);
    expect_result("GET http://host:443/ HTTP/1.0\r\n\r\n", 33,
            TAILNET_CONNECT_METHOD_NOT_ALLOWED);
    expect_result("CONNECT user@host:443 HTTP/1.1\r\n\r\n", 34,
            TAILNET_CONNECT_BAD_REQUEST);
    expect_result("CONNECT host:0 HTTP/1.1\r\n\r\n", 27,
            TAILNET_CONNECT_BAD_REQUEST);
    expect_result("CONNECT host:65536 HTTP/1.1\r\n\r\n", 31,
            TAILNET_CONNECT_BAD_REQUEST);
    expect_result("CONNECT fd7a:115c::1:443 HTTP/1.1\r\n\r\n", 37,
            TAILNET_CONNECT_BAD_REQUEST);
